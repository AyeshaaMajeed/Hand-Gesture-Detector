    ...
    def CotourFilter(self,img, area=1000.0):
        ...
        for cnt in contours:
            ...
            if larea > area:
                ...
                self.items.append(np.array([id, x, y, w, h, center], dtype=list))

                # store centers, and based on centers moving towards or away, set scale value between -1 and 1
    ...
    def DrawDetections(self, img, detections, offset =20, objCenter=True,objRectangle=True, label=[0,0], drawline=False):
        ...
        if len(detections) > 0:
            for ii,ll in zip(detections,label):
                ...
                if objCenter:
                    cv2.circle(limg, center, 2, (0, 0, 255), -1)
                if objRectangle:
                    cv2.rectangle(limg, (x-offset, y-offset), (x+w+offset, y+h+offset), (0, 0, 255), 1)
                    if ll[0] == id:
                        cv2.putText(limg, str(ll[1]), (x-offset+5, y-offset+15), self.font, 0.4, (0, 0, 255), 1, cv2.LINE_AA)
                        arr.append(center)
        ...
        return limg, arr # array of center of images: tmp
